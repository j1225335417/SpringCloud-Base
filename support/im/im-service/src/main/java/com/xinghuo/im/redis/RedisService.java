package com.xinghuo.im.redis;

import com.xinghuo.framework.common.model.UserLoginInfo;
import com.xinghuo.framework.redis.RedisClient;
import com.xinghuo.im.module.waiter.config.StoreProperties;
import com.xinghuo.im.netty.ImServerProperties;
import com.xinghuo.im.netty.server.WebSocketServer;
...
import static com.xinghuo.im.redis.RedisConstant.*;
...
public class RedisService {
    ...
    public void decrementServerConnectedCount(String address) {
        redisClient.zIncrementBy(NETTY_SERVER_ZSET_KEY, address, -1d);
    }
    ...
    public void removeNettyServer(String address) {
        redisClient.zRemove(NETTY_SERVER_ZSET_KEY, address);
    }
    ...
    public void addUserServerAddress(String userId, String address, int sysId) {
        String userAddressKey = getUserAddressKey(sysId, userId, address);
        redisClient.set(userAddressKey, "", imServerProperties.getClientRenewalSeconds());
    ...
    public void addUserServerAddressSet(String userId, String address, int sysId) {
        ...
        redisClient.expire(userAddressSetKey, USER_WEBSOCKET_ADDRESS_SET_TIME_DAYS, TimeUnit.DAYS);
    }
    ...
    public Set<String> userServerAddressSet(String userId, int sysId) {
        String userAddressSetKey = getUserAddressSetKey(sysId, userId);
        return redisClient.sMembers(userAddressSetKey);
    }
    ...
    public void removeUserServerAddressSet(String userId, String address, int sysId) {
        String userAddressSetKey = getUserAddressSetKey(sysId, userId);
        redisClient.sRemove(userAddressSetKey, address);
    }
    ...
    public void addNettyServerIfNotExist(String ip, int port) {
        redisClient.zAddNx(NETTY_SERVER_ZSET_KEY, AddressUtil.address(ip, port), 0L);
    }
    ...
    public void addStoreWaiter(String userId, String address, int sysId, Long storeId) {
        ...
        redisClient.set(storeWaiterHeartKey, "", imServerProperties.getClientRenewalSeconds());
    ...
    public Set<String> storeWaiterServerAddressSet(String userId, int sysId, Long storeId) {
        ...
        return redisClient.sMembers(storeWaiterAddressSetKey);
    }
    ...
    public boolean existStoreWaiter(Integer sysId, Long storeId, String waiterId) {
        String storeWaiterKey = getStoreWaiterZsetKey(sysId, storeId);
        return redisClient.zExists(storeWaiterKey, waiterId);
    }

    public long incrementStoreWaiter(Integer sysId, Long storeId, String waiterId) {
        String storeWaiterKey = getStoreWaiterZsetKey(sysId, storeId);
        return redisClient.zIncrementBy(storeWaiterKey, waiterId, 1d).longValue();
    }
    ...
    private String getUserAddressKey(int sysId, String userId, String address) {
        return RedisConstant.format(USER_WEBSOCKET_ADDRESS_KEY, sysId, userId, address);
    }

    private String getUserAddressSetKey(int sysId, String userId) {
        return RedisConstant.format(USER_WEBSOCKET_ADDRESS_SET_KEY, sysId, userId);
    }
